...
// Cube * cube;
GLint shaderProgram, shaderSkybox;
...
vec3 Window::cam_pos(0.0f, 0.0f, 20.0f);		// e  | Position of camera
...
void Window::leftClicking(vec3 last, vec3 curr) {
	...
	if (velocity > 0.0001f) {
		...
		//skybox->toWorld = rotate(mat4(1.0f), rotAngle / 180.0f * pi<float>(), rotAxis) * skybox->toWorld;

		cam_pos = mat3(rotate(mat4(1.0f), (rotAngle / 180.0f * pi<float>()), rotAxis)) * cam_pos;
		V = lookAt(cam_pos, cam_look_at, cam_up);

		//Window::V = rotate(mat4(1.0f), (rotAngle / 180.0f * pi<float>()), rotAxis) * Window::V;
	}
}